        ...
        >
          {category.map(({ text, icon, url }, index) => {
            return (
              ...
              >
                <div className={`w-6 relative ${activeCategory === text ? "text-orange-600" : "text-zinc-500"} text-xl`}>
                  <i className={`${icon}`}></i>
                </div>
                <div className={`grow shrink basis-0 ${activeCategory === text ? "text-orange-600 font-semibold" : "text-zinc-500  font-normal"} text-sm leading-tight`}>
                  {text}
                </div>
              </Link>
            );
